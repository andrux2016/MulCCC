...
$dsql->SetQuery("SELECT id,cointype,lastblock,reccheck,recfee,feetype,coinhost FROM ".$cfg_dbprefix."btctype WHERE coinsign=1");
$dsql->Execute();
...
$dsql->SetQuery("SELECT id FROM ".$cfg_dbprefix."btccash WHERE dealmark='0' AND adduser='0' AND checked='1'");
$dsql->Execute();
while($rech = $dsql->GetObject())
{
	
	$params=array($rech->id);
	$trans=coinQuery ("BTC","dealmark",$params);

	$rsup = $dsql->ExecuteNoneQuery("Update #@__btccash Set dealmark='".$trans['r']."',adduser='".$trans['r']."' where id = '".$rech->id."'"); 
