 ...
 * 会员权限管理
 *
 * @version        $Id: member_rank.php 1 12:37 2013年9月20日 SZ $
 */
...
if($dopost=='save')
{
    ...
    for(;$startID <= $endID;$startID++)
    {
		...
		$txfeeper = ${"txfeeper_".$startID};
        $dlfeeper = ${"dlfeeper_".$startID};
        if(isset(${"check_".$startID}))
        {
            if($rank>0)
            {
                $query = "UPDATE `#@__arcrank` SET membername='$name',money='$money',rank='$rank',scores='$scores',dlfeeper='$dlfeeper',txfeeper='$txfeeper' WHERE id='$id' ";            
            }
        }
        else
        {
