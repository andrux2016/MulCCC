 ...
 * 成交单操作
 *
 * @version        $Id: btc_deal_list.php 1 15:46 2013年9月20日Z SZ $
 */
...
if(isset($dopost))
{
    ...
    if ($dopost == 'delete')
    {
        ...
        foreach($nids as $n)
        {
            $query = "DELETE FROM `#@__btcdeal` WHERE id='$n'";
...
case 1:
  ...
  break;  
case 2:
  $addsql = "WHERE buyoid='".$oid."'";
  break;
case 3:
  $addsql = "WHERE selloid='".$oid."'";
  break;
case 4:
  $addsql = "WHERE (buserid='".GetUserID($oid)."' OR suserid='".GetUserID($oid)."')";
  break;
default:
  $addsql = '';
}
...
$sql = "SELECT * FROM #@__btcdeal $addsql ORDER BY `dealtime` DESC";
...
$tplfile = DEDEADMIN."/templets/btc_deal_list.htm";
...
function GetCoinBN()
{
	...
	{
		 $CoinBN .= "<input type=\"button\" name=\"ss".$row->id."\" value=\"".$row->cointype."\" style=\"width:50px;margin-right:6px\" onClick=\"location='btc_deal_list.php?coinid=".$row->id."';\"  class='np coolbg'/>";
	}
	return $CoinBN;
}