 ...
 * 会员管理操作
 *
 * @version        $Id: member_do.php 1 13:47 2013年9月10日 SZ $
 */
    ...
    if($fmdo=='yes')
    {
        ...
        if(!empty($id))
        {
            ...
            if($rs > 0)
            {
				...
				$dsql->ExecuteNoneQuery("DELETE FROM `#@__btccoin` WHERE userid='$id' ");//删除金钱
				$dsql->ExecuteNoneQuery("DELETE FROM `#@__btcorder` WHERE userid='$id' ");//删除挂单
...
else if ($dopost=='edituser')
{
	...
	if($googleCannel==1) $pwdsql.=",google=''";//取消google验证
	if($txpwd!="") $pwdsql.=",txpwd='".md5($txpwd)."'";//取消google验证
    ...
    $query = "UPDATE `#@__member_person` SET
            tel = '$tel'
            WHERE mid='$id' ";
            $rs = $dsql->ExecuteNoneQuery2($query);
	...
	{
		$cname="coin_".$rtype->cointype;
		$cfree="free_".$rtype->cointype;
		$query = "UPDATE `#@__btccoin` SET c_deposit = '". $$cname ."',c_freeze = '". $$cfree ."',edittime = '". time() ."' WHERE userid='$id' AND coinid='".$rtype->id."'";
		$rcoin = $dsql->ExecuteNoneQuery2($query);
		if($rcoin==0){
			$query = "Insert Into `#@__btccoin`(userid,coinid,cointype,c_deposit,c_freeze,edittime) Values('$id','".$rtype->id."','".$rtype->cointype."','". $$cname ."','". $$cfree ."','". time() ."')";
...
else if ($dopost=="memberlogin")
{
    ...
    if(empty($jumpurl)) header("location:..".DEDEMEMBER."/index.php");
