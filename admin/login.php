...
if($dopost=='login')
{
    ...
    if((false)){
		...
		if($_SESSION[$userip.'err']>5 || $_SESSION[$userid.'err']>5)
		{
			...
			ShowMsg("错误次数过多，稍后再试！","-1");
			exit();
		}
		if($_SESSION[$userid.'err']>8)
			{
				$rsup = $dsql->ExecuteNoneQuery("Update #@__admin Set usertype=0 where userid = '".$userid."' "); 
				$rsnew = $dsql->ExecuteNoneQuery("insert into #@__log(adminid,filename,method,query,cip,dtime) values('0','login.php','err-".$userid."-".$pwd."','login','$userip','".time()."')");
				ShowMsg("错误次数过多，稍后再试！","-1");
				exit();
			}
        ...
        if(!empty($userid) && !empty($pwd))
        {
            ...
            else
            {
				...
				$_SESSION[$userid.'err']=$_SESSION[$userid.'err']+1;
				$errtimes=6-$_SESSION[$userip.'err'];
				//ShowMsg('你的密码错误!',-1,0,1000);
				$rsnew = $dsql->ExecuteNoneQuery("insert into #@__log(adminid,filename,method,query,cip,dtime) values('0','login.php','err-".$userid."-".$pwd."','login','$userip','".time()."')");
				ShowMsg("你的密码错误！您还有".$errtimes."次机会","login.php");
				exit;
            }
        }
