...
if($dopost=="save")
{
    ...
    for( ;$startID <= $endID; $startID++)
    {
        ...
        if(isset(${'check_'.$startID}))
        {
			...
			$query = "DELETE FROM #@__moneycard_record WHERE aid='$aid' ";
    ...
    if(isset($check_new) && $numbers!='')
    {
		...
		foreach ($rechargeCodes as $key => $value){ 
			...
			$ctid=$value['no'];
			$cardid=$value['pwd'];
			$query = "INSERT INTO #@__moneycard_record(ctid,coinid,mid,cardid,isexp,money,stime,mtime,cardnote) VALUES('{$ctid}','{$coinid}','".$cuserLogin->getUserId()."','{$cardid}','1','{$money_new}',".time().",'{$mtime}','{$cardnote}');";
			$dsql->ExecuteNoneQuery($query);
		} 
		header("Content-Type: text/html; charset={$cfg_soft_lang}");
    	echo "<script> alert('成功生成".$numbers."张充值卡！'); </script>";
    }
}
...
$sql = "Select * From #@__moneycard_record ORDER BY `stime` DESC";

$dlist = new DataListCP();

if(isset($aid)) $dlist->SetParameter("aid",$aid);

$tplfile = DEDEADMIN."/templets/member_recharge_cards.htm";
