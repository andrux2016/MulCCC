...
class membermodel
{
    ...
    */
    function getForm($type = 'post', $value = '', $admintype='membermodel2')
    {
            ...
            foreach($dtp->CTags as $tagid=>$tag)
            {
                if($tag->GetAtt('autofield'))
                {
                    ...
                    公开所有联系方式</p>';
                                ...
                                }else if($tag->GetName()=='mobile'){
									
									$formstring .='';
                                 }else if($tag->GetName()=='tel'){
									if($value[$tag->GetName()]=="") $bdtel="绑定手机";
									else $bdtel="修改手机";
									$formstring .='<p><label>'.$tag->GetAtt('itemname').'：</label><input type="text" class="intxt" style="width: 100px;" id="tel" value="'.$value[$tag->GetName()].'" name="tel"  disabled > <a href="edit_baseinfo_btc.php">'.$bdtel.'
    </a></p>';
                                 } else if ($tag->GetAtt('type') == 'img')
                                {
                                    ...
                                    $fvalue = htmlspecialchars($value[$tag->GetName()],ENT_QUOTES);
										...
										$imgstrng = "<p>123<label>{$labelname}：</label><input type='text' name='$fieldname' value='$fvalue' id='$fieldname' style='width:300px'  class='text' /> <input name='".$fieldname."_bt' class='inputbut' type='button' value='浏览...' onClick=\"SelectImage('addcontent.$fieldname','big')\" />\r\n</p>";
        ...
        $formstring .= "<input type=\"hidden\" name=\"dede_fields\" id=\"dede_fields\" value=\"".$formfields."\" />\n";
        $formstring .= "<input type=\"hidden\" name=\"dede_fieldshash\" id=\"dede_fieldshash\" value=\"".md5($formfields.$cfg_cookie_encode)."\" />";
        return $formstring;
    }//end func getForm
