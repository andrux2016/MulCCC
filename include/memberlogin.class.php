 ...
 * 会员登录类
 *
 * @version        $Id: userlogin.class.php 1 15:59 2013年9月5日 SZ $
 */
...
function CheckTxPdw()
{
	...
	$nowtxpwd = $row['txpwd'];
	if($nowtxpwd==""){
		ShowMsg("请先完善账户安全资料！","edit_baseinfo_btc.php");
		exit();
	}
}
...
class MemberLogin
{
        ...
        if(empty($this->M_ID))
        {
            ...
            if(is_array($this->fields)){
				...
				*/
				...
				$this->M_StrCode = $this->fields['StrCode'];
                $rtel = $dsql->GetOne("Select tel From #@__member_person where mid='{$this->M_ID}' ");
				$this->M_Tel = $rtel['tel'];
				...
				//读取级别
				$ranks = $dsql->GetOne("Select membername,dlfeeper,txfeeper From `#@__arcrank` where rank='{$this->M_Rank}' ");
				if( is_array($ranks) )
                {
                    $this->M_Vip = $ranks['membername'];
					$this->M_FeePer = $ranks['dlfeeper'];
					$this->M_TXFeePer = $ranks['txfeeper'];
                }

				//超时退出
				if((time()-$_COOKIE["DedeLoginTime"]) > 1800 || $this->M_Rank==0) 
				{
					$this->ResetUser();
					//exit();
				}else{
					//PutCookie('DedeUserID',$cfg_ml->M_ID,3600);
    ...
    {
		global $dsql;
		$rarc = $dsql->GetOne("Select * From #@__arcrank Where scores<=".$this->M_Scores." Order by rank desc");
		if(is_array($rarc)){
			if($rarc['rank'] != $this->M_Rank){
				$rsup = $dsql->ExecuteNoneQuery("Update #@__member Set rank='".$rarc['rank']."' where mid = '".$this->M_ID."'"); 
			}
		}
		return $rsup;
    }
    ...
    function CheckUser(&$loginuser, $loginpwd)
    {
        ...
        $row = $dsql->GetOne("SELECT mid,rank,matt,pwd,logintime FROM `#@__member` WHERE userid LIKE '$loginuser' ");
        if(is_array($row))
        {
            if($this->GetShortPwd($row['pwd']) != $this->GetEncodePwd($loginpwd))
            {
               /*$userip=$_SERVER["REMOTE_ADDR"];
				@session_id(preg_match("/^[0-9a-zA-Z.]+$/",$loginuser));
				@session_start();
				$_SESSION[$userip.'err']++;
				$_SESSION[$loginuser.'err']++;*/
				return -1;
            }
            else
            {
