...
function Dialog(content, options)
{
    var defaults = { // 默认值。 
        ...
        closeText:'确 认', // 关闭按钮文字，若不想显示关闭按钮请通过CSS设置其display为none 
        draggable:true,     // 是否移动 
        modal:true,         // 是否是模态对话框 
        center:true,        // 是否居中。 
        fixed:true,         // 是否跟随页面滚动。
    ...
    var barHtml = !options.showTitle ? '' :
        '<div class="bar"><button type="submit" class="btn-2 close" >' + options.closeText + '</button></div>';
    ...
    this.setContent = function(c)
    {
        ...
        if('object' == typeof(c))
        {
			...
			case 'postbuy':
				div.html('加载中...');
				if($("#"+$("#b_moneytype").val()).html()<($("#b_btccount").val()*$("#b_uprice").val())){
					div.html(" 余额不足! ");resetPos();
				}else{
					$.post(c.value,{coinid:$("#b_coinid").val(), moneyid:$("#b_moneyid").val(), $market:$("#b_market").val(), bkage:$("#b_bkage").val(), uprice:$("#b_uprice").val(), btccount:$("#b_btccount").val(), dealtype:0},function(html){
			...
			case 'postsell':
				div.html('加载中...');
				if($("#"+$("#s_cointype").val()).html()<$("#s_btccount").val()){
					div.html(" 余额不足! ");resetPos();
				}else{
					$.post(c.value,{coinid:$("#s_coinid").val(), moneyid:$("#s_moneyid").val(), $market:$("#s_market").val(), bkage:$("#s_bkage").val(), uprice:$("#s_uprice").val(), btccount:$("#s_btccount").val(), dealtype:1},function(html){
						div.html(html);resetPos();
					});
				}
				break;
