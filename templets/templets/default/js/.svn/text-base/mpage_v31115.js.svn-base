function mpage_operator(server_time, cur_symbol, uid) {
	...
	var main_ask_bid_list_obj = {
		...
		reload_ask_bid_grid: function() {
			...
			for (var i = 0; i < ask_length; i++){
				...
				$(".l_divAskList").append("<tr><td class='sell'>卖("+(i+1)+")</td><td>￥"+ this.ask_list[i].rate.toFixed(2) +"</td><td>"+this.ask_list[i].symbol_l.toFixed(8)+"</td><td title='"+ this.ask_list[i].count + "单'><span  class='buySpan' style='width:"+(this.ask_list[i].symbol_r/mastcount*100)+"%'></span></td>");
				if(i<5) $(".l_divList").prepend("<tr><td class='sell'>卖("+(i+1)+")</td><td>￥"+ this.ask_list[i].rate.toFixed(2) +"</td><td>"+this.ask_list[i].symbol_l.toFixed(8)+"</td><td title='"+ this.ask_list[i].count + "单'><span  class='buySpan' style='width:"+(this.ask_list[i].symbol_r/mastcount*100)+"%'></span></td>");
			
				
				//$(".l_divAskList").append("<tr><td>"+ this.ask_list[i].rate.toFixed(trade_global.digits) +"</td><td>"+this.ask_list[i].symbol_l.toFixed(trade_global.digits)+"</td><td>"+(this.ask_list[i].rate*this.ask_list[i].symbol_l).toFixed(trade_global.digits)+"</td><td title='"+ this.ask_list[i].count + "单'><div style='background-color:#990000;height:20px;width:"+(this.ask_list[i].symbol_r/mastcount*100)+"%'></div></td></tr>");
				//if(i<5) $(".l_divList").prepend("<tr><td >"+ this.ask_list[i].rate.toFixed(trade_global.digits) +"</td><td>"+this.ask_list[i].symbol_l.toFixed(trade_global.digits)+"</td><td>"+(this.ask_list[i].rate*this.ask_list[i].symbol_l).toFixed(trade_global.digits)+"</td><td title='"+ this.ask_list[i].count + "单'><div style='background-color:#990000;height:20px;width:"+(this.ask_list[i].symbol_r/mastcount*100)+"%'></div></td></tr>");
			...
			for (var i = 0; i < bid_length; i++){
				...
				$(".l_divBidList").append("<tr><td class='buy'>买("+(i+1)+")</td><td>￥"+ this.bid_list[i].rate.toFixed(2) +"</td><td>"+this.bid_list[i].symbol_l.toFixed(8)+"</td><td title='"+ this.bid_list[i].count + "单'><span  class='sellSpan' style='width:"+(this.bid_list[i].symbol_r/mastcount*100)+"%'></span></td>");
				if(i<5) $(".l_divList").append("<tr><td  class='buy'>买("+(i+1)+")</td><td>￥"+ this.bid_list[i].rate.toFixed(2) +"</td><td>"+this.bid_list[i].symbol_l.toFixed(8)+"</td><td title='"+ this.bid_list[i].count + "单'><span  class='sellSpan' style='width:"+(this.bid_list[i].symbol_r/mastcount*100)+"%'></span></td>");
			
				
				
				//$(".l_divBidList").append("<tr><td>"+ this.bid_list[i].rate.toFixed(trade_global.digits) +"</td><td>"+this.bid_list[i].symbol_l.toFixed(trade_global.digits)+"</td><td>"+(this.bid_list[i].rate*this.bid_list[i].symbol_l).toFixed(trade_global.digits)+"</td><td title='"+ this.bid_list[i].count + "单'><div style='background-color:#009900;height:20px;width:"+(this.bid_list[i].symbol_r/mastcount*100)+"%'></div></td></tr>");	
				//if(i<5) $(".l_divList").append("<tr><td >"+ this.bid_list[i].rate.toFixed(trade_global.digits) +"</td><td>"+this.bid_list[i].symbol_l.toFixed(trade_global.digits)+"</td><td>"+(this.bid_list[i].rate*this.bid_list[i].symbol_l).toFixed(trade_global.digits)+"</td><td title='"+ this.bid_list[i].count + "单'><div style='background-color:#009900;height:20px;width:"+(this.bid_list[i].symbol_r/mastcount*100)+"%'></div></td></tr>");
	...
	var main_history_grid_obj = {
		...
		init: function(history_data) {
			...
			//	$(".hqHistoryList").append("<tr><td>"+ stime +"</td><td>"+this.history_data[i].order+"</td><td>"+(this.history_data[i].rate.toFixed(trade_global.digits))+"</td><td>"+(this.history_data[i].amount_l.toFixed(trade_global.digits))+"</td><td>"+ this.history_data[i].amount_r + "</td></tr>");
				...
				success: function(data, state) {
					...
					if (json_res && json_res.result && json_res.history) {
						for (var i = 0; i < json_res.history.length; i++) {
							...
							node_his.order = json_res.history[i].order == 1 ? "<font color='#FF6600'>卖出</font>": "<font color='#FFA200'>买入</font>";
							...
							node_his.date = $.format.date(json_res.history[i].date * 1000, 'HH:mm:ss');
							...
							if (node_his) {
								...
								$("#divHistoryList").append("<li style=\"width: 180px;\">"+ node_his.date +"</li><li style=\"width: 80px;\">"+node_his.order+"</li><li style=\"width: 120px;\">"+node_his.rate+"</li><li style=\"width: 140px;\">"+ node_his.amount_l +"</li><li style=\"width: 140px;\">"+ node_his.amount_r.toFixed(trade_global.digits)+ "</li>");
	...
	var page_obj = {
		...
		on_request_ask_bid: function(type, rate, vol) {
			...
			/*var dlg_processing = apprise("<table  width='60' align='center'><tr><td>请稍后...</td></tr></table>", {
				animate: false,
				buttonShow: false,
				textOk: "确定"
			});*/
...
$.ajax({
	...
	success: function(data, textStatus) {
		...
		if (json_res.records) {
			...
			view_code += "---------------------------------------------------";
			/*view_code += "<hr/>";*/
			view_code += "<div class='btc-tanchuang'>";
			view_code += "<table id='tableRecords' width='120' >";
		...
		if (json_res.pending) {
			...
			view_code += "---------------------------------------------------"
			/*view_code += "<hr/>";*/
			view_code += "<table  id='tablePending' width='120'>";
...
on_input_ask_vol: function() {
	...
	var fixed_num = num_need_fix(element_vol.value, trade_global.digits);
	if (fixed_num) {
	...
	var fixed_num = num_need_fix(element_amount.value, trade_global.digits);
	if (fixed_num) {
...
on_input_bid_amount: function() {
	...
	vol /= 10000;
	element_vol.value = vol;
	var fixed_num = num_need_fix(vol, trade_global.digits);
	if (fixed_num) {
		element_vol.value = fixed_num
	};
	return true
	...
	$.ajax({
		...
		success: function(data, state) {
			...
			if (json_res && json_res.result) {
				...
				pb_vol.innerHTML = json_res.ticker.vol.toFixed(trade_global.digits)
