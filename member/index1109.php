...
if(isset($_GET['upcache']) || !file_exists('index.html'))
{
	...
	foreach($dtypearr as $key => $typemune){
		...
		if($key==$cvid) $convertName .= "<a class='btc8-abase btc8-avist' href='".$cfg_cmsurl."/?".$typemune['cointype']."_".$typemune['moneytype']."'>".$typemune['cointype']."/".$typemune['moneytype']."<br>".$rateAllArr['last_rate']."</a>";
		else $convertName .= "<a class='btc8-abase' href='".$cfg_cmsurl."/?".$typemune['cointype']."_".$typemune['moneytype']."'>".$typemune['cointype']."/".$typemune['moneytype']."<br>".$rateAllArr['last_rate']."</a>";
	}
	//echo $coinid;
...
if($_COOKIE["DedeUserID"]!="" || !isset($_COOKIE["DedeUserID"])){
	...
	foreach ($cfg_arrcoin as $value){
		if($value['1']>0 || $value['2']>0) $coinhtml.="<div style='width:100%'>".$value['0'].":<font color='#FF0000'>".number_format($value['1'], 4, '.', '')."</font>&nbsp;&nbsp;冻结：".number_format($value['2'], 4, '.', '')."</div>";
		//$coinhtml.="<div><label class='fleft'>".$value['0']."余额：</label><span class='coininfo'>".($value['1']/1)."</span><span>冻结：".($value['2']/1)."</span></div>";
		if($value['0']==$cointype) $coindeposit=number_format($value['1'], 4, '.', '');
		if($value['0']==$moneytype) $moneydeposit=number_format($value['1'], 4, '.', '');
		$coinvol+=$value['4'];
	}
	...
	foreach ($orderArr as $value){
		...
		$ordershow .= "<tr><td style=\"width:15%;text-align:center\">". $dtype ."</td><td style=\"width:35%;text-align:center\">".($value['uprice']/1)."</td><td style='width:28%; text-align:center'>".($value['btccount']/1)."</td><td style=\"width: 20%\" title='撤单'><div class='trash' style='text-align:center;cursor:pointer' title='撤单' onclick='_page.obj.cancel_order(\"".$cointype."_".$moneytype."\",\"".$value['oid']."\");'>撤单</div></td></tr>";
	}
