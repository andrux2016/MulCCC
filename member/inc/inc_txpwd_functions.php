<?php   if(!defined('DEDEMEMBER')) exit("SZ");
/**
 * 密码函数
 * 
 * @version        $Id: inc_pwd_functions.php 1 15:18 2010年9月1日 SZ $
 */
...
function newmail($mid, $userid, $mailto, $type, $send)
{
    ...
    {
        $key = md5($randval);
        $sql = "INSERT INTO `#@__txpwd_tmp` (`mid` ,`membername` ,`pwd` ,`mailtime`)VALUES ('$mid', '$userid',  '$key', '$mailtime');";
    ...
    {
        $key = md5($randval);
        $sql = "UPDATE `#@__txpwd_tmp` SET `pwd` = '$key',mailtime = '$mailtime'  WHERE `mid` ='$mid';";
        if($db->ExecuteNoneQuery($sql))
        {
            if($send == 'Y')
            {
...
function sn($mid,$userid,$mailto, $send = 'Y')
{
    ...
    $tptim= (60*10);
    $dtime = time();
    $sql = "SELECT * FROM #@__txpwd_tmp WHERE mid = '$mid'";
    $row = $db->GetOne($sql);
    if(!is_array($row))
    {
        //发送新邮件；
