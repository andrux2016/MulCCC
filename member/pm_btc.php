 ...
 * 会员短消息
 * 
 * @version        $Id: pm_btc.php 1 8:38 2013年8月9日Z SZ $
 */
...
if($dopost=='send')
{
    ...
    include_once(dirname(__FILE__).'/templets/pm-send_btc.htm');
    exit();
}
/*-----------------------
function __read(){  }
...
else if($dopost=='read')
{
    ...
    include_once(dirname(__FILE__).'/templets/pm-read_btc.htm');
    exit();
}
/*-----------------------
function __savesend(){  }
...
else if($dopost=='savesend')
{
    ...
    ShowMsg("成功发送一条信息!","pm_btc.php?dopost=outbox");
    exit();
}
/*-----------------------
function __del(){  }
...
else if($dopost=='del')
{
    ...
    $dsql->ExecuteNoneQuery($query);
    ShowMsg("成功删除指定的消息!","pm_btc.php?folder=".$folder);
    exit();
}
/*-----------------------
function __man(){  }
...
else
{
    ...
    $dlist->SetTemplate(DEDEMEMBER.'/templets/pm-main_btc.htm');
