...
if($dealtype==0){//买入**************************************************
		...
		$dsql->SetQuery("SELECT * FROM ".$cfg_dbprefix."btcorder WHERE dealtype=1 AND coinid='".$applist['coinid']."' AND moneyid='".$applist['moneyid']."' AND market='".$applist['market']."' AND uprice <= ".$applist['uprice']." ORDER BY uprice asc,ordertime");//执行SQL语句
