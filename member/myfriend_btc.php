 ...
 * 我的好友
 *
 * @version        $Id: myfriend_btc.php 1 17:55 2013年8月6日Z SZ $
 */
...
if($dopost=='upsta')
{
    ...
    if($sta=='good')
    {
        ShowMsg("成功把指定好友设为关注好友！","myfriend_btc.php?ftype=1");
    }
    else if($sta=='bad')
    {
        ShowMsg("成功把指定好友放入黑名单！","myfriend_btc.php?ftype=-1");
    }
    else
    {
        ShowMsg("成功把指定好友转为普通好友！","myfriend_btc.php");
    }
    exit();
}
...
else if($dopost=='del')
{
    ...
    ShowMsg("成功删除所选的好友！","myfriend_btc.php?ftype=".$ftype);
    exit();
}
//浏览
else{
    ...
    $dlist->SetTemplate(dirname(__FILE__).'/templets/myfriend_btc.htm');
