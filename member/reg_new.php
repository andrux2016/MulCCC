<?php
/* @version        $Id: reg_new.php 1 8:38 2013年9月9日 SZ $
 */
...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
       ...
       /*if(preg_match("/1/", $safe_gdopen)){
        ...
        //$userid = preg_replace("#[^0-9A-Za-z-]#", "", trim($userid));
		
        $pwd = safe_string(trim($userpwd));
        $pwdc = safe_string(trim($userpwdok));
		$txpwd = safe_string(trim($txPwd));
        $txpwdc = safe_string(trim($txPwdok));
        ...
        if(strlen($userid) > 50 || strlen($userid) < $cfg_mb_idmin)
        {
            ShowMsg('你的用户名长度不合适，不允许注册！', '-1');
            exit();
        }
        if(strlen($pwd) < $cfg_mb_pwdmin)
        {
            ShowMsg("你的登陆密码过短，不允许注册！","-1");
            exit();
        }
		/*if(strlen($txpwd) < $cfg_mb_pwdmin || strlen($pwd) < $cfg_mb_pwdmin)
        {
            ShowMsg("你的登陆密码或提现密码过短，不允许注册！","-1");
            exit();
        }
        if($pwdc != $pwd || $txpwd != $txpwdc)
        {
		...
		if($cfg_welcome=="Y"){
			...
			$row = $dsql->GetOne("SELECT * FROM #@__moneycard_welcome WHERE ctid='$ctid' AND cardid='$password' AND coinid='".$rcoin['id']."'");
        ...
        /*$inQuery = "INSERT INTO `#@__member` (`mtype` ,`userid` ,`jsuserid` ,`pwd` ,`txpwd` ,`uname` ,`sex` ,`rank` ,`money` ,`email` ,`scores` ,
        `matt`, `spacesta` ,`face`,`safequestion`,`safeanswer` ,`jointime` ,`joinip` ,`logintime` ,`loginip` )
       VALUES ('$mtype','$userid','$invite','$pwd','$txpwd','$uname','$sex','10','$dfmoney','$email','$dfscores',
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";*/
	   $inQuery = "INSERT INTO `#@__member` (`mtype` ,`userid` ,`jsuserid` ,`pwd` ,`uname` ,`sex` ,`rank` ,`money` ,`email` ,`scores` ,
        `matt`, `spacesta` ,`face` ,`jointime` ,`joinip` ,`logintime` ,`loginip` )
       VALUES ('$mtype','$userid','$invite','$pwd','$uname','$sex','10','$dfmoney','$email','$dfscores',
       '0','$spaceSta','','$jointime','$joinip','$logintime','$loginip'); ";
        if($dsql->ExecuteNoneQuery($inQuery))
        {
			...
			//如果是内测邀请码，充值
			if($cfg_welcome=="Y"){
				$dsql->ExecuteNoneQuery("UPDATE #@__moneycard_welcome SET uid='".$mid."',isexp='-1',utime='".time()."' WHERE ctid='$ctid' ");
				echo "UPDATE #@__moneycard_welcome SET uid='".$mid."',isexp='-1',utime='".time()."' WHERE ctid='$ctid'";
				$membertcoin = "INSERT INTO `#@__btccoin` (userid,coinid,cointype,c_deposit,c_freeze,edittime) VALUES ('".$mid."','".$rcoin['id']."','".$strArr[1]."','".$hasMoney."','0','". time() ."'); ";
				$dsql->ExecuteNoneQuery($membertcoin);
			}
            ...
            //写入其它默认数据
            $dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid','btcsz','http://www.btcsz.com'); ");
            ...
            if($cfg_mb_reginfo == 'Y' && $spaceSta >=0)
            {
				...
				if($cfg_mb_spacesta==-10)
				{
					...
					require_once(DEDEMEMBER."/templets/edit_fullinfo_btc.htm");		
				}
                exit;
            } 
        } else {
