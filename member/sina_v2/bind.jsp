...
<%
String sina_user_id=RequestUtil.getStringParam("sina_user_id",request,"");
System.out.println(GlobalMethod.formatYYYYMMDDHHMM(System.currentTimeMillis())+"@@@@@@" + request.getRequestURI()+"********sina_user_id=" + sina_user_id);
String code=RequestUtil.getStringParam("code",request,"");
if(sina_user_id.equals("")){
...
return;
	...
	String publishInfo = sina_user_id+"@sina";
	
	//处理已经登录的用户，直接绑定
	System.out.println("处理已经登录的用户，直接绑定="+session.getAttribute("userInfo"));
	if(session.getAttribute("userInfo")!=null){
		UserInfo user = (UserInfo)session.getAttribute("userInfo");
		user.setPublishInfo(publishInfo);
		UserInfoProxy.getInstance().updateObj(user);
		System.out.println("处理已经登录的用户，直接绑定="+session.getAttribute("userInfo"));
		//插入抽奖号
		//LotteryOrder lotteryOrder = LotteryOrderProxy.getInstance().getTotteryOrderByUserId(user.getUserId());
		
		String condition = "begin_time_long<="+System.currentTimeMillis()+" and end_time_long>="+System.currentTimeMillis() +" and cate_id=213 limit 1";
		String curLotteryProIdStr = DBOper.getFieldValue("product", "pro_id", condition);
		
		if(curLotteryProIdStr!=null&&!curLotteryProIdStr.equals("")){
			int curLotteryProId=Integer.parseInt(curLotteryProIdStr);
			LotteryOrder lotteryOrder = LotteryOrderProxy.getInstance().getTotteryOrderByUserId(user.getUserId(),Integer.parseInt(curLotteryProIdStr));
	 		if(lotteryOrder!=null){//参与了抽奖，并且存在当前抽奖信息
				if(curLotteryProId==lotteryOrder.getProId().intValue()){
					String lotteryCodeLength=GlobalMethod.getDictText("system_info","lottery_code_length");
					String lotteryCode = LotteryCodeProxy.getCode(5);
					if(lotteryCodeLength!=null&&!lotteryCodeLength.trim().equals("")){
						lotteryCode = LotteryCodeProxy.getCode(Integer.parseInt(lotteryCodeLength));
					}
					
					LotteryCodeCache.codeList.add(lotteryCode);
					LotteryCode lotteryCodeObj = new LotteryCode();
					lotteryCodeObj.setLotteryCode(lotteryCode);
					lotteryCodeObj.setUserId(user.getUserId());
					lotteryCodeObj.setUserName(user.getUserName());
					int proId = Integer.parseInt(curLotteryProIdStr);
					lotteryCodeObj.setProId(proId);
					lotteryCodeObj.setInviteType(2);//邦定微薄
					lotteryCodeObj.setInviteDesc("微薄绑定");
					lotteryCodeObj.setProTitle(lotteryOrder.getOrderTitle());
					LotteryCodeProxy.getInstance().createLotteryCode2(lotteryCodeObj);
					
					System.out.println("微薄绑定后，插入获取的抽奖号成功!");
					%>
					<script>location.href="/lottery/lotterSuccess.jsp?orderId=<%=lotteryOrder.getOrderId() %>&bindSuccess=1";</script>
					<%
					return;
				}
	 		}
        ...
        %>
       <script>location.href="/charge/";</script>
        <%
		return;
	}else{
		List list = DBOper.queryInfo("from UserInfo where publishInfo='"+publishInfo+"'",1);
		if(list.size()>0){//已存在，返回原来页面
				UserInfo sinaUser = (UserInfo)list.get(0);
				session.setAttribute("userName",sinaUser.getUserName());
		        session.setAttribute("userInfo",sinaUser);
		        session.setAttribute("UserInfo",sinaUser);
		        String logintourl = session.getAttribute("logintourl")==null?"/":session.getAttribute("logintourl").toString();
		      
		        %>
		       <script>location.href="<%=logintourl%>";</script>
		        <%
		        return;
...
$(document).ready(function() {
	...
	function checkRegFrm(){
			...
			$.post(
		    	...
		    	{token:regToken,randNum:randNum,name: name,email: email,passwd:password1,mobile:mobile,code:addCode},
		    	function(data){
			    	var html = "";
			    	var json = eval("("+data+")");
			    		if(json.para1==1){
			    		...
			    		}else if(json.para1==4){
			    			$('#regTokenErr').html('<img src="/images/ico_c.gif" width="14" height="14" hspace="3" align="absmiddle" />验证码错误!');
			    		}else{
...
function loadimage(idName,id){
	document.getElementById(idName).src = "/token-"+id+"?"+Math.random(); 
}
</script>
</head>
<body>
...
onfocus="aClass(this,'bindTxtRedBod');rClass(this,'border_solid1');"/><span class="color01" id="regMobileInit"></span></td>
          ...
          <tr>
                <td align="right" class="tl">验证码</td>
                <td>
 				<input type="text" name="regToken" id="regToken" class="border_solid1" style="height:30px;width:60px;margin-right:5px;"/><img id="regtokenimg" src="/token-3" width="60" height="20" border="1" align="absmiddle">&nbsp;&nbsp;<a href="javascript:loadimage('regtokenimg','3');"><font class=pt95>看不清,换一张</font></a>
				<span class="color01" id="regTokenErr"><xweb:error name="regCodeErr"/></span>
				</td>
          </tr>
          ...
          <tr>
            <td><input type="hidden" id="regUser.userEmail" name="regUser.userEmail" value=""/>
             ...
             <input type="hidden" name="destURL" value="/sina_v2/regSuccess.jsp"/>
            </td>
            <td>
