...
while($row = $dsql->GetObject())
{
	if($row->id == $coinid){
		if($row->feetype==1){
			...
			$feeshow=($row->coinfee*$cfg_ml->M_TXFeePer*100)."%";
...
if($action == "do") {
	...
	if($cfg_ml->M_Tel!=""){
		...
		if(!isset($emailcode)){
			$out=array('code'=>0,'msg'=>$emailcode.'短信验证码过期！');
			showJson($out); 
			exit();
		}
		...
		if(!isset($_SESSION[$cfg_ml->M_LoginID.'vd'])){
			showJson('Email验证码过期！',"-1"); 
			exit();
		}
		if($emailcode!=$_SESSION[$cfg_ml->M_LoginID.'vd']){
			
			$userip=$cfg_ml->M_LoginID;
			$_SESSION[$userip.'err']=$_SESSION[$userip.'err']+1;
			$errtimes=5-$_SESSION[$userip.'err'];
			showJson($emailcode."-".$_SESSION[$cfg_ml->M_LoginID.'vd']."Email验证码错误！您还有".$errtimes."次机会","-1");
			exit();
		}
	}
...
//$tpl->LoadTemplate(DEDEMEMBER.'/templets/cash_'.$cointype.'.htm');
