...
echo "content：";
echo $content = 'billno'.$billno.'currencytype'.$currency_type.'amount'.$amount.'date'.$mydate.'succ'.$succ.'ipsbillno'.$ipsbillno.'retencodetype'.$retEncodeType;

//请在该字段中放置商户登陆merchant.ips.com.cn下载的证书
echo "<br>证书为：";
echo $cert = 'GDgLwwdK270Qj1w4xho8lyTpRQZV9Jm5x4NwWOTThUa4fMhEBK9jOXFrKRT6xhlJuU2FEa89ov0ryyjfJuuPkcGzO5CeVx5ZIrkkt1aBlZV36ySvHOMcNv8rncRiy3DQ';
echo "<br>MD5加密得到：";
echo $signature_1ocal = md5($content . $cert);

echo "<br>转换小写后：";
echo $signature_1ocal = strtolower($signature_1ocal);
echo "<br>";
	//----------------------------------------------------
	//  判断交易是否成功
	//  See the successful flag of this transaction
	//----------------------------------------------------
	echo "两个签名为:".$signature_1ocal." = ". iconv("UTF-8","gb2312",$signature);
	echo "<br>";
